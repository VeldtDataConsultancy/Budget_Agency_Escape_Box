...
// ENUMERATORS
typedef enum {Timer_Init, Timer, Edoc_Init, Edoc} lcdStateType;
lcdStateType lcdState = Timer_Init;
...
// GLOBAL VARIABLES
uint8_t       mp3ToPlay = 0;    // Mp3 number to be played as soon as an Mp3 is requested.
uint8_t       scriptStep = 0;   // Number of the step used inside the script.
...
unsigned long startTime;        // Start time of the game. Gets set in the setup phase.
int32_t clockSec;               // Current second on the clock.
int32_t oldSec = 0;             // Previous second on the clock.
char timeString[20];            // Current time as a character string.

String eDocs[20] = {"Parse Complete 1", "Parse Complete 2", "Parse Complete 3"};
...
void loop() {
  ...
  // State in helping out the phone.
  switch (phoneState) {
    case Idle_Init:
      mp3.stop();
      phoneState = Idle;
